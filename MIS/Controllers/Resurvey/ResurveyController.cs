...
{
    public class ResurveyController : BaseController
    {
        ResurveyDataImportReportService objReportService = new ResurveyDataImportReportService();
        CommonFunction common = new CommonFunction();
        # region File Import

        public ActionResult FileImport(NHRPResurveyDetail mdl)
        {
            ...
            string id = null;
            id = mdl.DISTRICT_CD.ConvertToString();
        ...
        {
            NHRPResurveyDetail model = new NHRPResurveyDetail();
            ...
            try
            {
                ...
                DataTable dtFileStatusReport = new DataTable();
                string filepath = Server.MapPath("~/Files/ResurveyCSV/" + SearchValue + "/");
        ...
        {
            ResurveyFileImportService fileImpService = new ResurveyFileImportService();
            ...
            try
            {
                if (Directory.Exists(initialPath))
                {
                    ...
                    if (hidJsonFileName != null && hidJsonFileName.Length > 0)
                    {
                        ...
                        {
                            if (Directory.GetFiles(initialPath).Length == 10)
                            {
                                List<SortedDictionary<dynamic, dynamic>> totalSurveys = new List<SortedDictionary<dynamic, dynamic>>();
                                int num_rows = 0;
                                int num_cols = 0;
                                ...
                                var collection = from t1 in tableHouseOwners.AsEnumerable()
                                                 ...
                                                 select new
                                                 {
                                                     ...
                                                     ward = t1["ward"],
                                                     enum_ar = t1["enum_ar"],
                                                     house_no = t1["house_no"],
                                                     enum_id = t1["enum_id"],
                                                     tole = t1["tole"],
                                                     own_num = t1["own_num"],
                                                     resn_for_griev = t1["resn_for_griev"],//Resurvey 
                                                     has_old_idslip = t1["has_old_idslip"],//Resurvey 
                                                     old_idslip_photo = t1["old_idslip_photo"],//Resurvey 
                                                     resn_for_left = t1["resn_for_left"],//Resurvey 
                                                     resn_oth = t1["resn_oth"],//Resurvey 
                                                     vdc_old = t1["vdc_old"],//Resurvey 
                                                     ward_old = t1["ward_old"],//Resurvey 
                                                     district_name = t1["district_name"],//Resurvey 
                                                     vdc_name = t1["vdc_name"],//Resurvey 
                                                     old_vdc_name = t1["old_vdc_name"],//Resurvey                                                   
                                                     sim_tel = t1["sim_tel"],
                                                     comment = t1["comment"],
                                                     uuid = t2["uuid"],
                                                     st_time = t2["st_time"],
                                ...
                                result.Columns.Add("resn_for_griev", typeof(string)); //Resurvey 
                                result.Columns.Add("has_old_idslip", typeof(string)); //Resurvey 
                                result.Columns.Add("old_idslip_photo", typeof(string));//Resurvey 
                                result.Columns.Add("resn_for_left", typeof(string));//Resurvey 
                                result.Columns.Add("resn_oth", typeof(string));//Resurvey 
                                result.Columns.Add("vdc_old", typeof(string));//Resurvey 
                                result.Columns.Add("ward_old", typeof(string));//Resurvey 
                                result.Columns.Add("district_name", typeof(string));//Resurvey 
                                result.Columns.Add("vdc_name", typeof(string));//Resurvey 
                                result.Columns.Add("old_vdc_name", typeof(string));//Resurvey 
                                ...
                                foreach (var item in collection)
                                {
                                    result.Rows.Add(item.ona_id, item.district, item.vdc, item.ward,
                                        item.enum_ar, item.house_no, item.enum_id,item.tole, item.own_num,item.resn_for_griev,item.has_old_idslip,item.old_idslip_photo,
                                        item.resn_for_left,item.resn_oth,item.vdc_old,item.ward_old, item.district_name,item.vdc_name,item.old_vdc_name,

                                        item.comment, item.uuid, item.st_time, item.en_time, item.today, item.imei, item.imsi, item.sim, item.sim_tel, item.sub_time);
                                }
                                ...
                                #region Grievance Code for CSV
                                string[,] valuesGrievanceDetail = LoadCsv(initialPath + "/Grievance_House_Owner.csv");
                                //string[,] valuesOwnersDetail = LoadCsvTab(initialPath + "/House_Owner.csv"));
                                num_rows = valuesGrievanceDetail.GetUpperBound(0) + 1;
                                num_cols = valuesGrievanceDetail.GetUpperBound(1) + 1;
                                DataTable tableGrievanceDetail = new DataTable();
                                for (int c = 0; c < num_cols; c++)
                                {
                                    tableGrievanceDetail.Columns.Add(valuesGrievanceDetail[0, c].Replace("\"", ""));
                                }
                                for (int r = 1; r < num_rows; r++)
                                {
                                    tableGrievanceDetail.Rows.Add();
                                    for (int c = 0; c < num_cols; c++)
                                    {
                                        tableGrievanceDetail.Rows[r - 1][c] = valuesGrievanceDetail[r, c].Replace("\"", "");
                                    }
                                }
                                #endregion
                                ...
                                foreach (DataRow dr in result.Rows)
                                {
                                    ...
                                    List<SortedDictionary<dynamic, dynamic>> lstGrievanceDetail = new List<SortedDictionary<dynamic, dynamic>>();


                                    DataRow[] drSelectGrievanceDetail = tableGrievanceDetail.Select("ona_id='" + dr["ona_id"] + "'");
                                    foreach (DataRow dr1 in drSelectGrievanceDetail)
                                    {
                                        SortedDictionary<dynamic, dynamic> indOwner = new SortedDictionary<dynamic, dynamic>();
                                        foreach (DataColumn dc in tableGrievanceDetail.Columns)
                                        {
                                            indOwner.Add(dc.ColumnName, dr1[dc.ColumnName].ToString());
                                        }
                                        lstGrievanceDetail.Add(indOwner);
                                    }
                                    ...
                                    foreach (DataRow dr3 in drSelectBuilding)
                                    {
                                        ...
                                        DataRow[] drSelectFamily = tableFamilyCount.Select("ona_id='" + dr3["ona_id"] + "' and house_no='" + dr3["house_no"] + "'");//house_sn=building structure number
                                        foreach (DataRow dr4 in drSelectFamily)
                                        {
                                            ...
                                            DataRow[] drIndividual = tableIndividualCount.Select("ona_id='" + dr4["ona_id"] + "' and house_no='" + dr4["house_no"] + "'");//howner_sn=Family serial num
                                            ...
                                            DataRow[] drDeath = tableDeathCount.Select("ona_id='" + dr4["ona_id"] + "' and house_no='" + dr4["house_no"] + "'");
                                            ...
                                            DataRow[] drDestructed = tableDestructedCount.Select("ona_id='" + dr4["ona_id"] + "' and house_no='" + dr4["house_no"] + "'");
                                    ...
                                    indSurvey.Add("Grievance", lstGrievanceDetail);
        ...
        public ActionResult FileImport(string hidJsonFileName, string hidDistrictName)
        {
            ...
            string id = string.Empty;
            string exc = string.Empty; 
            string initialPath1 = Server.MapPath("~/Files/ResurveyCSV/" + hidDistrictName);
            try
            {
                ...
                if (hidJsonFileName == "All")
                {
                    ...
                    string filepath = Server.MapPath("~/Files/ResurveyCSV/" + hidDistrictName + "/");
                    if (Directory.Exists(filepath))
                    {
                        ...
                        foreach (string s in jsonFiles1)
                        {
                            ...
                            initialPath = Server.MapPath("~/Files/ResurveyCSV/" + hidDistrictName + "/" + s);
                ...
                else
                {
                    ...
                    {
                        initialPath = Server.MapPath("~/Files/ResurveyCSV/" + hidDistrictName + "/" + hidJsonFileName);
        ...
        #region OtherDTL
        private void InsertMigrationOthData(string initialPath, string hidJsonFileName)
        {
            ResurveyFileImportService fileImpService = new ResurveyFileImportService();
            FormCollection fc = new FormCollection();
            string id = string.Empty;
                       ...
                       // if (!fileListInDB.Contains(hidJsonFileName))
                        {
                            if (Directory.GetFiles(initialPath).Length == 10)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        string batchID = fileImpService.GetBatchIDFromFileName(hidJsonFileName);
                                        success = fileImpService.SaveDataCSVOther(hidJsonFileName, totalSurveys, "I", batchID, out surveyCount, out houseCount, out householdCount, out memberCount, out exc);
                                    }
                                    else
                                    {
                                        success = fileImpService.SaveDataCSVOther(hidJsonFileName, totalSurveys, "I", "", out surveyCount, out houseCount, out householdCount, out memberCount, out exc);
                                    }
                                    //success = fileImpService.SaveDataCSV(hidJsonFileName, totalSurveys, "I", out surveyCount, out houseCount, out householdCount, out memberCount, out exc);
                                    ViewData["surveyCnt"] = surveyCount;
                                    ViewData["houseCnt"] = houseCount;
        ...
        [HttpPost]
        public ActionResult FileImportOth(string hidJsonFileName, string hidDistrictName)
        {
            FileImportService fileImpService = new FileImportService();
            FormCollection fc = new FormCollection();
            string id = string.Empty;
            ...
            try
            {
                ...
                if (hidJsonFileName == "All")
                {
                    ...
                    if (Directory.Exists(filepath))
                    {
                        ...
                        foreach (string s in jsonFiles1)
                        {
                            ...
                            initialPath = Server.MapPath("~/Files/ResurveyCSV/" + hidDistrictName + "/" + s);
                            InsertMigrationOthData(initialPath, s);
                        }
                        // dtFileStatusReport1 = objReportService.GetDataImportReportForDashboard1(jsonFiles);
                    }
                ...
                else
                {
                    ...
                    {
                        initialPath = Server.MapPath("~/Files/ResurveyCSV/" + hidDistrictName + "/" + hidJsonFileName);
                        InsertMigrationOthData(initialPath, hidJsonFileName);
                    }
