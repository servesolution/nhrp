...
{
    public class NHRPFileImportController : BaseController
    {
                    ...
                    if (hidJsonFileName != null && hidJsonFileName.Length > 0)
                    {
                        ...
                        if (!fileListInDB.Contains(hidJsonFileName))
                        {
                            if (Directory.GetFiles(initialPath).Length == 9)
                            {
                                ...
                                foreach (DataColumn col in tableHouseOwners.Columns)
                                {
                                    if (col.ColumnName == "rel_mention")
                                    {
                                        tableHouseOwners.Columns["rel_mention"].ColumnName = "relatnod";
                                    }
                                    if (col.ColumnName == "howner_reason")
                                    {
                                        tableHouseOwners.Columns["howner_reason"].ColumnName = "reason";
                                    }
                                    if (col.ColumnName == "reason_mention")
                                    {
                                        tableHouseOwners.Columns["reason_mention"].ColumnName = "reasonod";
                                    }
                                }
