...
{
    public class InspectionImportJsonService
    {
        public bool SaveInspectionData(List<SortedDictionary<dynamic, dynamic>> InspectionInfo, string filename)
        {
            ...
            InspectionHigherLevelUploadService ObjSaveHigherInspectionServ = new InspectionHigherLevelUploadService();
                ...
                using (ServiceFactory service = new ServiceFactory())
                {
                    try
                    {
                        ...
                        foreach (var InspectionItem in InspectionList)
                        {
                            ...
                            objMst.NRA_DEFINED_CD = InspectionItem.ContainsKey("PA_NUMBER") ? Utils.ConvertToString(InspectionItem.First(s => s.Key == "PA_NUMBER").Value) : null;
                            objDesign.NRA_DEFINED_CD = InspectionItem.ContainsKey("PA_NUMBER") ? Utils.ConvertToString(InspectionItem.First(s => s.Key == "PA_NUMBER").Value) : null;
                            objApplication.NRA_DEFINED_CD = InspectionItem.ContainsKey("PA_NUMBER") ? Utils.ConvertToString(InspectionItem.First(s => s.Key == "PA_NUMBER").Value) : null;
                            objPapr.NRA_DEFINED_CD = InspectionItem.ContainsKey("PA_NUMBER") ? Utils.ConvertToString(InspectionItem.First(s => s.Key == "PA_NUMBER").Value) : null;
                            objPapr.SERIAL_NUMBER = InspectionItem.ContainsKey("FORM_NUMBER") ? Utils.ConvertToString(InspectionItem.First(s => s.Key == "FORM_NUMBER").Value) : null;
                    ...
                    NoteRemarksExit:
                        ...
                        if (objAdditionalService.CheckDuplicate(objMst.NRA_DEFINED_CD.ConvertToString(), objPapr.SERIAL_NUMBER.ConvertToString(), inspectionNumber))
                        {
                            string reason = "Duplicate Inspection";
                            bool saveDuplicate = objAdditionalService.saveDuplicateInspectionData(objMst.NRA_DEFINED_CD.ConvertToString(),
                                objPapr.SERIAL_NUMBER.ConvertToString().ConvertToString(),
                                objDesign.BENF_FULL_NAME.ConvertToString(),
                                objDesign.DISTRICT_CD.ConvertToString(),
                                objDesign.VDC_MUN_CD.ConvertToString(),
                                objDesign.WARD.ConvertToString(), "file name", reason);
                            goto saveDuplicateInspectionDataExit;
                        }
                        else if (objAdditionalService.CheckUpdate(objMst.NRA_DEFINED_CD.ConvertToString(), objPapr.SERIAL_NUMBER.ConvertToString(), inspectionNumber))
                        {


                            objUpdateService.UpdateInspectionData( InspectionInfo,  filename);
                            goto saveDuplicateInspectionDataExit;
                        }

                        //check inspection higher level upload
                        if (inspectionNumber!="1")
                        {
                            if (objAdditionalService.CheckHigherInspection(objMst.NRA_DEFINED_CD.ConvertToString(), objPapr.SERIAL_NUMBER.ConvertToString(), inspectionNumber))
                            {
                                ObjSaveHigherInspectionServ.saveHigherInspection(InspectionInfo, filename);
                                goto saveDuplicateInspectionDataExit;
                            }
                            else
                            {
                                string reasonToSave = "No application for given inspection";
                                bool saveDuplicate = objAdditionalService.saveDuplicateInspectionData(objMst.NRA_DEFINED_CD.ConvertToString(),
                                 objPapr.SERIAL_NUMBER.ConvertToString().ConvertToString(),
                                 objDesign.BENF_FULL_NAME.ConvertToString(),
                                 objDesign.DISTRICT_CD.ConvertToString(),
                                 objDesign.VDC_MUN_CD.ConvertToString(),
                                 objDesign.WARD.ConvertToString(), "file name", reasonToSave);
                            }
                            goto saveDuplicateInspectionDataExit;
                        }
                        ...
                        QueryResult qrPaperDtl = service.SubmitChanges("PR_NHRS_INSPECTION_PAPER_DTL",
                             ...
                             "I",
                                                   ...
                                                   objPapr.SERIAL_NUMBER.ConvertToString(),          //serial number/form number
                   ...
                   saveDuplicateInspectionDataExit:
